...
describe('endHabit', () => {
	it("sets the habit's end date to today", async () => {
		...
		const dto = expectOk(await endHabit(deps, { habitId }));
		expect(dto.endDate).toBe('2024-06-10');
		expect(dto.active).toBe(true); // the end day itself is still within range
	});

	it('makes the habit inactive from the following day', async () => {
		const { deps, habitId } = await setup('2024-06-10');
		await endHabit(deps, { habitId });
	...
	it('persists the ended habit', async () => {
	...
	it('rejects ending before the start date', async () => {
		const { deps, habitId } = await setup('2024-05-01'); // before the 2024-06-01 start date
		const error = expectErr(await endHabit(deps, { habitId }));
		expect(error).toBeDefined();
	});

	it('returns HabitNotFound for an unknown id', async () => {
		const { deps } = await setup();
		const error = expectErr(await endHabit(deps, { habitId: HabitId.generate().value }));
		expect(error).toBeInstanceOf(HabitNotFound);
	});

	it('rejects a malformed habit id', async () => {
		const { deps } = await setup();
		const error = expectErr(await endHabit(deps, { habitId: 'not-a-uuid' }));
